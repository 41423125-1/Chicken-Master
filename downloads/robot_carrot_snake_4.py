...
def seek_and_collect():
    ...
    if target:
        ...
        if not current_path:
            # 即使 BFS 找到路徑，但如果中間有問題，這裡也做一次檢查
            print("⚠️ 找不到到達 carrot 的路徑，跳過此回合。")
            timer.set_timeout(seek_and_collect, 500)
            return
